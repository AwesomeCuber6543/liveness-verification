...
export default function PassiveVerification() {
  ...
  useEffect(() => {
    ...
    console.log("[Page] Mounting passive verification, starting...");
    v.start();

    return () => {
      console.log("[Page] Unmounting passive verification, stopping...");
      v.stop();
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Navigate to result page when verification completes
  useEffect(() => {
    if (v.step === "RESULT" && v.result) {
      ...
      window.location.href = `/verify/passive/result?${params.toString()}`;
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [v.step, v.result]);
